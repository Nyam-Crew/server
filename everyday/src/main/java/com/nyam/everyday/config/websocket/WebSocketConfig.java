...
import com.nyam.everyday.config.websocket.interceptor.StompAuthConnectInterceptor;
...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
  ...
  private final StompAuthConnectInterceptor stompAuthConnectInterceptor;

  @Override
  public void registerStompEndpoints(StompEndpointRegistry registry) {
    registry.addEndpoint("/ws") // 클라이언트가 연결할 엔드포인트
        .setAllowedOrigins("http://localhost:5173") // CORS 허용
        // .setHandshakeHandler(new CustomHandshakeHandler())  // 직접 생성한 Handler 추가
        // .addInterceptors(new CustomHandshakeInterceptor())  // 직접 생성한 Interceptor 추가
        .withSockJS();
  }
