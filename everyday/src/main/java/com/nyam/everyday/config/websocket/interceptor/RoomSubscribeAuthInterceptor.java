...
import com.nyam.everyday.module.chatting.service.ChattingRoomService;
...
/**
 * [한눈에 보기]
 * 사용자가 "/topic/room/{방번호}" 같은 토픽을 구독하려고 할 때,
 * 정말 그 방을 볼 자격이 있는지 확인하는 가드(문지기)입니다.
 *
 * [작동 흐름]
 * 1) 클라이언트가 STOMP SUBSCRIBE 요청을 보냅니다. (예: "/topic/room/1")
 * 2) 이 인터셉터가 요청을 가로챕니다.
 * 3) 현재 사용자(Principal)의 ID와 구독하려는 목적지(dest)를 읽습니다.
 * 4) RoomMembershipService로 "해당 방의 멤버인가?"를 확인합니다.
 * 5) 멤버가 아니면 예외를 던져 구독을 차단합니다. (클라이언트는 ERROR 프레임 수신)
 *
 * [중요 개념]
 * - Principal: "이 연결은 누구의 것인가"를 나타내는 사용자 식별 정보입니다.
 *   HandshakeHandler 등에서 세팅된 값이 여기서 acc.getUser().getName()으로 도달합니다.
 * - Destination(목적지): 구독하려는 토픽 경로입니다. (예: "/topic/room/1")
 * - RoomMembershipService: "사용자가 이 방의 멤버인가?"를 판단하는 도메인 서비스입니다.
 *
 * [등록 방법]
 * WebSocketConfig.configureClientInboundChannel(...)에서 registration.interceptors(...)로 등록합니다.
 */
...
public class RoomSubscribeAuthInterceptor implements ChannelInterceptor {
  ...
  /**
   * 클라이언트가 서버로 메시지를 보내기 "직전"에 호출됩니다.
   * 여기서는 SUBSCRIBE(구독) 요청만 골라서 권한을 체크합니다.
   *
   * @param message  클라이언트가 보낸 STOMP 프레임(헤더/바디 포함)
   * @param channel  내부 전달 채널(직접 사용할 일은 거의 없습니다)
   * @return         문제가 없으면 그대로 message를 반환하여 다음 단계로 진행합니다.
   *                 권한이 없으면 예외를 던져 구독을 차단합니다.
   */
  @Override
  public Message<?> preSend(Message<?> message, MessageChannel channel) {
    ...
    if (StompCommand.SUBSCRIBE.equals(acc.getCommand())) {
      ...
      if (dest != null && dest.startsWith("/topic/room/")) {
        ...
        log.info("구독 요청 수신, 유저번호 : {}, 연결을 요청한 방 번호 : {}", userId, roomId);

        // 핵심 권한 체크: 사용자 ID가 있고, 그 사용자가 해당 방의 멤버인지 확인합니다.
        // 실제 구현은 DB/캐시 조회로 대체됩니다.
        boolean allowed = userId != null && chattingRoomService.isMember(Long.parseLong(userId), Long.parseLong(roomId));

        // 멤버가 아니면 구독을 막습니다. 클라이언트는 STOMP ERROR 프레임을 받고, 구독은 완료되지 않습니다.
        if (!allowed) {
          log.info("인증되지 않아 요청을 거절합니다");
