...
public interface TeamMemberStatusRepository extends JpaRepository<TeamMemberStatus, Long> {
    ...
    @Query("SELECT tms FROM TeamMemberStatus tms " +
            "JOIN FETCH tms.member " + // Member 엔티티를 함께 조회 (EAGER 로딩)
            "WHERE tms.team.teamId = :teamId AND tms.status = :status")
    List<TeamMemberStatus> findAllByTeam_TeamIdAndStatusWithMember(
    ...
    @Query("SELECT tms.team.teamId FROM TeamMemberStatus tms WHERE tms.member.memberId = :memberId AND tms.status = com.nyam.everyday.module.team.enums.ParticipationStatus.APPROVED")
